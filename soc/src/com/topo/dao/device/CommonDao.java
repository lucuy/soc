...
public interface CommonDao {
	//添加设备
	Integer addObject(Device device) throws SQLException;
	//修改设备
	Integer editObject(Device device) throws SQLException;
	//查询所有内网的设备列表
	List<Device> queryAllDevice(Map map)throws SQLException;
	//根据设备ID到设备
	Device queryDeviceById(long id)throws SQLException;
	//根据设备ID删除设备信息，可以传1个或多个参数
	void deleteDeviceByIds(int... ids) throws SQLException;
	//根据设备ID查询与该设备像关联的设备
	List<Device> queryAssociationDeviceByID(long id)throws SQLException;
	
	//根据设备类型ID查询设备信息，可以传1个或多个参数
		public int queryDeviceByDeviceCategoryID(long id)throws SQLException;
	/**
     * 按条件分页查询设备信息
     */
    public List<Device> query(Map map, int startRow, int pageSize)throws SQLException;
