package com.soc.service.knowledge.exportsecurity;
...
public class ExcelSecurity
{
    ...
    public void export(List<Map> list, String title)
    {
        ...
        cell.setCellStyle(csHeader);
        cell.setCellType(HSSFCell.CELL_TYPE_STRING);
        sheet.setColumnWidth(iColNumber++, (256 * 35));
        cell.setCellValue("描 述");
        // 2、填充基本信息内容
        for (int i = 0; i < list.size(); i++)
        {
            ...
            cell.setCellType(HSSFCell.CELL_TYPE_STRING);
            cell.setCellValue(map.get("SECURITY_ID").toString());
            
            cell = row2.createCell(iColNumber++);
            cell.setCellStyle(csBody);
            cell.setCellType(HSSFCell.CELL_TYPE_STRING);
            cell.setCellValue(map.get("SECURITY_TITLE").toString());
            
            cell = row2.createCell(iColNumber++);
            cell.setCellStyle(csBody);
            cell.setCellType(HSSFCell.CELL_TYPE_STRING);
            cell.setCellValue(map.get("SECURITY_PUBLISHER").toString());
            
            cell = row2.createCell(iColNumber++);
            cell.setCellStyle(csBody);
            cell.setCellType(HSSFCell.CELL_TYPE_STRING);
            // 时间处理
            int point = map.get("SECURITY_DATE").toString().indexOf(".");
            if (point == -1)
            {
                cell.setCellValue(map.get("SECURITY_DATE").toString());
            }
            else
            {
                cell.setCellValue(map.get("SECURITY_DATE").toString().substring(0, point));
            }
            
            cell = row2.createCell(iColNumber++);
            cell.setCellStyle(csBody);
            cell.setCellType(HSSFCell.CELL_TYPE_STRING);
            cell.setCellValue(map.get("SECURITY_SOURCE").toString());
            
            String details = map.get("SECURITY_DETAILS").toString();
            cell = row2.createCell(iColNumber++);
            cell.setCellStyle(csBody);
            cell.setCellType(HSSFCell.CELL_TYPE_STRING);
            cell.setCellValue(details.replaceAll("<p>", "").replaceAll("</p>", ""));
    ...
    public static void main(String[] args) {
		String s = "<p>123</p>";
		System.err.println(s.replaceAll("<p>", "").replaceAll("</p>", ""));
