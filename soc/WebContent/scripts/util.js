...
function addRadio_b(name, id, idd, div,hid) {
	$("#"+name).empty();
	$("#"+idd).empty();
	$("#scriptName-"+id).attr("checked", true);
	$("#"+idd).val(name);
	$("#"+hid).val(id);
	$("#"+div).dialog('close');
}
...
function initRadio1(type, radioName) {
...
//想select中添加radio
function add_radio(selectId, radioDiv) {
	 // 确定按钮
	$("#"+selectId).empty();
	$("#"+radioDiv+" input[type='radio']:checked").each(
			   function() {
				 var node = $(this).parent().next().children("a");
			        $("#"+selectId).append("<option value=\"" + $(this).val() + "\">" + node.text() + "</option>"); 
			         }
			);
		// var node = $("#"+radioDiv+" input[type='radio']:checked");
		// alert(node.html());
	//$("#"+selectId).append("<option value=\"" + node.val() + "\">" + node.text() + "</option>"); 
	$("#"+radioDiv).dialog('close');
	}