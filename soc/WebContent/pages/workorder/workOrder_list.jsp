...
var rege =/^([^`~!@#$%^&*()=|{}':;, \\\[\].<>\/?~！@#￥……&*（）—|{}【】‘；：”“'。\"，、？])*$/;
		...
		function query(){
			...
			var falg = $("#falg").val();
			//var manyStatus = $("#manyStatus").val();
			//location.href = "${ctx}/workOrder/quckQuery.action?keyword="+encodeURI(encodeURI(keyword,"utf-8"))+"&manyStatus=+"+manyStatus;
			location.href = "${ctx}/workOrder/quckQuery.action?keyword="+encodeURI(encodeURI(keyword,"utf-8"))+"&falg=+"+falg;
		}
		...
		function deleteWarn(){
			...
			if (confirm("确认执行操作吗？")) {
				location.href="${ctx}/workOrder/deleteById.action?result="+result+"&manyStatus="+$("#manyStatus").val();
				}	
		}
...
<body style="margin-top:2px;">
	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0"
		...
		<div class="box">
			...
			<input  type="hidden" id="falg" value="${falg}" name="falg" class="jianju" />
				...
				<div id="dataList">
					<table width="100%" border="0" cellspacing="1" cellpadding="0"
						...
						<tr height="28" class="biaoti">
							...
							<th width="15%" class="biaoti">派发人</th>
							<th width="15%" class="biaoti">责任人</th>
						...
						<c:forEach items="${listWork}" var="Ls">
							<tr>
								...
								<td valign="middle">
									<c:if test="${Ls.status==0}">未处理<a href="#">关闭工单</a></c:if>
									<c:if test="${Ls.status==1}">已处理</c:if>
									<c:if test="${Ls.status==2}">已关闭</c:if>
