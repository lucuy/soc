...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function loadpage(){
			...
			//为漏洞编号下拉列表传值
			var str = "<s:property value="#request.queryId.leakNum"/>";
			var arr = str.split(",");
			...
			for(var i=0;i<arr.length;i++){
				...
				Option.value=arr[i];
				Holes.appendChild(Option);
				Option.selected=true; 
			}
			//为受影响的服务器进行传值
			var strp = "<s:property value="#request.queryId.influenceInterface"/>";
			var arrp = strp.split(",");
			var Ports = document.getElementById("Ports");
			for(var i=0;i<arrp.length;i++){
				var Option = document.createElement("OPTION");
				Option.appendChild(document.createTextNode(arrp[i]));
				Option.value=arrp[i];
				Ports.appendChild(Option);
				Option.selected=true;
			}
			//为受影响的服务器端口进行传值
			var stri = "<s:property value="#request.queryId.influenceSys"/>";
			var arri = stri.split(",");
			var Ports = document.getElementById("OSs");
			for(var i=0;i<arri.length;i++){
				var Option = document.createElement("OPTION");
				Option.appendChild(document.createTextNode(arri[i]));
				Option.value=arri[i];
				Ports.appendChild(Option);
				Option.selected=true;
			}
			...
			//判断新漏洞类型
		    document.getElementById("NewHoleType").value="<s:property value="#request.queryId.leakType"/>";
		    
			//判断新漏洞等级
			var leakLevel = document.getElementsByName("warn.leakLevel");
			for(var i=0;i<leakLevel.length;i++){
				if(leakLevel[i].value=="<s:property value="#request.queryId.leakLevel"/>"){
					leakLevel[i].checked = true;
				}
			}
			//判断状态
			var status = document.getElementsByName("warn.status");
			for(var i=0;i<status.length;i++){
				if(status[i].value=="<s:property value="#request.queryId.status"/>"){
					status[i].checked = true;
				}
			}
			//判断严重程度
			var seriousLevel = document.getElementsByName("warn.seriousLevel");
			for(var i=0;i<seriousLevel.length;i++){
				if(seriousLevel[i].value=="<s:property value="#request.queryId.seriousLevel"/>"){
					seriousLevel[i].checked = true;
				}
			}
			//判断更改的是漏洞还是蠕虫现实不同的页面
			var warnType = document.getElementsByName("warn.warnType");
			for(var i=0;i<warnType.length;i++){
				if(warnType[i].value=="<s:property value="#request.queryId.warnType"/>"){
					if(warnType[i].value==2){
						warnType[i].checked = true;
		...
		function AddItemOSs()
		{
			...
			if (document.frmPost.PK_TypeId.value==0)
			{
				alert("不能添加未知操作系统");
			...
			{
				for(var n=0;n<document.frmPost.BrandId.options.length;n++)
				{
					if (document.frmPost.BrandId.options[n].selected == true)
					{
					oOption.text=document.frmPost.BrandId.options[n].text+" ";
					}
				}
				for(var n=0;n<document.frmPost.PK_TypeId.options.length;n++)
				{
					if (document.frmPost.PK_TypeId.options[n].selected == true)
					{
					oOption.text+=document.frmPost.PK_TypeId.options[n].text+" ";
					}
				}
				for(var n=0;n<document.frmPost.EditionId.options.length;n++)
				{
					if (document.frmPost.EditionId.options[n].selected == true)
					{
					oOption.text+=document.frmPost.EditionId.options[n].text+" ";
					}
				}
				for(var n=0;n<document.frmPost.NumId.options.length;n++)
				{
					if (document.frmPost.NumId.options[n].selected == true)
					{
					oOption.text+=document.frmPost.NumId.options[n].text+" ";
					}
				}
				
				for(var n=0;n<document.frmPost.PK_CpuId.options.length;n++)
				{
					if (document.frmPost.PK_CpuId.options[n].selected == true)
					{
					oOption.text+=document.frmPost.PK_CpuId.options[n].text;
					}
				}
				oOption.value=document.frmPost.PK_TypeId.value+"|||"+document.frmPost.EditionId.value+"|||"+document.frmPost.NumId.value+"|||"+document.frmPost.BrandId.value+"|||"+document.frmPost.PK_CpuId.value+"|||"+oOption.text;
				oOption.selected=true;
				
				i = document.frmPost.OSs.length;
				...
				if (i>0)
				{
					...
					for (j=0;j<=i-1;j++)
					{
						if (document.frmPost.OSs.options[j].text == oOption.text)
						{
							alert("不能重复输入操作系统");
							document.frmPost.PK_TypeId.focus();
							return false;
				...
				//document.frmPost.OSs.add(oOption);
				document.frmPost.PK_TypeId.value="";
			}
		}
		//添加漏洞编号
  ...
  <body onload="loadpage()">
    	<form action="${ctx}/warn/updateById.action?warnId=<s:property value="#request.queryId.warnId"/>" method="post" name="frmPost" id="frmPost">
    		<!-- 主table -->
			...
			style="margin-left: 4px; margin-top: 0px">
    			<tr>
    				<td  width="50%" valign="top">
    					<!-- 左侧table -->
    					<div class="framDiv" style="height: 500px">
    						<table width="100%" border="0" cellspacing="1" cellpadding="0">
	    						<tr>
									<td class='r2title' colspan='4'><b>查看或修改预警</b></td>
								</tr>
				    			<tr>
								...
								<tr>
									...
									<td width="70%" class="msg" colspan="2" style="padding-left:10px" align="left">
										<input type="text" class="validate[required,custom[validateLength],custom[spechars]] text-input " id="warnName" value="<s:property value="#request.queryId.warnName"/>" name="warn.warnName" size="68"
										 />
									</td>
								</tr>
								<!-- 空行 -->
								...
								<tr>
									...
									<td width="70%" colspan="2" style="padding-left:10px" align="left">
										<textarea rows="5" name="warn.warnDescription" cols="50"
											class="validate[required]"  id="warnDescription" ><s:property value="#request.queryId.warnDescription"/></textarea>
									</td>
								</tr>
								<!-- 空行 -->
								<tr>
								...
								<tr>
									...
									<td width="70%" colspan="2" style="padding-left:10px" align="left">
										<textarea rows="5" name="warn.warnHarm" id="warnHarm" class="validate[required]" cols="50"><s:property value="#request.queryId.warnHarm"/></textarea>
									</td>
								</tr>
								<!-- 空行 -->
								<tr>
								...
								<tr id="idNewHoleName" >
									...
									<td width="70%" colspan="2" style="padding-left:10px" align="left">
										<input type="text"  id="leakName" class="validate[required]" value="<s:property value="#request.queryId.leakName"/>" name="warn.leakName"  maxlength="100" size="67.50">
									</td>
								</tr>
								<!-- 空行 -->
								<tr>
								...
								<tr id="idNewHoleType" >
									...
									<td width="70%"  colspan="2" style="padding-left:10px" align="left">
										<select name="warn.leakType" id="NewHoleType" style="width:370px" >
								...
								<tr>
									...
									<td width="70%"  colspan="2" style="padding-left:10px" align="left">
										<textarea rows="5"  id="solution" class="validate[required]"  name="warn.solution" cols="50"><s:property value="#request.queryId.solution"/></textarea>
									</td>
								</tr>
    						</table>
    					</div>
    				...
    				<td valign="top">
    					...
    					<div class="framDiv" style="height: 500px">
    						<table width="100%" border="0" cellspacing="1" cellpadding="0">
    							<tr>
									<td class='r2title' colspan='4'><b></b></td>
								</tr>
    							<!-- 空行 -->
								<tr>
											<td class="td_detail_separator"></td>
								...
								<tr>
									...
									<td width="70%"  colspan="2" style="padding-left:10px" align="left">
										<input type="text"  name="warn.publicDate" 
										value="<s:date name="#request.queryId.publicDate"
										format="yyyy-MM-dd" />"
										 size="67.50"  onclick="WdatePicker()" readonly />
									</td>
								</tr>
								<!-- 空行 -->
								<tr>
								...
								<tr>
									...
									<td width="70%" colspan="2" style="padding-left:10px" align="left">
										<input type="text"   id="source" class="validate[required]"  value="<s:property value="#request.queryId.source"/>"  size="67.50" name="warn.source"  maxlength="20">
									</td>
								</tr>
								<!-- 空行 -->
								<tr>
								...
								<tr>
									...
									<td width="70%" colspan="2" style="padding-left:10px" align="left">
										<input type="text" id="publisher" class="validate[required]"  size="67.50" value="<s:property value="#request.queryId.publisher"/>" name="warn.publisher"   maxlength="50">
									</td>
								</tr>
								<!-- 空行 -->
								<tr>
								...
								<tr>
									...
									<td width="30%">
										<INPUT onClick="AddItemOSs()" type=button  class="btnadd"  width="80" style="padding-left:10px" align="left">
										<INPUT onClick="DeleteItem('OSs')" type=button  class="btndel" width="80">
										<br>
										<select name="warn.influenceSys" id="OSs" class="validate[required]" style="width: 370px" multiple size="5"  >
								...
								<tr>
									...
									<td width="40%"  colspan="2" style="padding-left:10px" align="left">
										...
										BUGTRAQ
										<input type="text" value="" name="ForecastHoleNumber"  maxlength="20"  ><br>
										...
										<br>
										<select name="warn.leakNum" class="validate[required]" id="Holes" style="width: 370px" multiple size="3">
								...
								<tr>
									<td height="20" align="right" class="labelColor">
										受影响的服务器端口：
									</td>
									...
									<td width="70%"  colspan="2" style="padding-left:10px" align="left">
										...
										UDP
										<input type="text" value="" name="ForecastPort" class="form1Input" maxlength="100" ><br>
    		...
    		<table align="right">
				<tr align="right">
					<td height="30" colspan="4">
						<input type="submit"  value="修　改" class="btnyh"  width="80">
						<input type="button" name="back" value="返  回" class="btnyh" class="btnyh" onClick="location='${ctx}/warn/query.action'" width="80">
					</td>
				</tr>
			</table>
    	</form>
