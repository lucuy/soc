...
<script language="javascript">
	...
	function query() {
		...
		location.href = "${ctx}/monitor/customMonitorList.action?keyword="
				+ encodeURIComponent(encodeURIComponent(keyword));
	}
	function addMonitorDialog() {
		location.href = "${ctx}/pages/monitor/monitor_value_chart.jsp";
	}
	...
	function deleteUser() {
		...
		if (confirm("确认执行操作吗？")) {
			location.href = "${ctx}/monitor/deleteCustom.action?ids="
					+ ids;
		}
	}
...
<body style="margin-top:2px;">
	...
	<s:form action="customMonitorList.action" namespace="/monitor" method="post"
							...
							<div id="dataList">
								<table width="100%" border="0" cellspacing="1" cellpadding="0"
									...
									<tr height="28" class="biaoti">
										<td width="5%" class="biaoti"><input type="checkbox"
											id="chkAll" name="chkAll" class="check-box not_checked" /></td>
										<th width="15%" class="biaoti">图表名称</th>
										<th width="15%" class="biaoti">创建时间</th>
										<th width="15%" class="biaoti">修改时间</th>
										<th width="15%" class="biaoti">修改者</th>
										<th width="15%" class="biaoti">描述</th>
									</tr>
									</thead>
									<tbody>
									<s:iterator value="monitorList" status="stat">
										<tr>
											<td class="biaocm" valign="middle"><input
												type="checkbox" name="ids" id="${monitorId}"
												value="${monitorId}" class="check-box" /></td>
											<td valign="middle" > <a href="selectMonitorById.action?monitorId=${monitorId}"   title="${monitorName }" >${monitorName}</a></td>
											<td valign="middle"><s:date
													name="monitorCreateDatetime"
													format="yyyy-MM-dd HH:mm:ss" />
											</td>
												<td valign="middle"><s:date
													name="monitorUpdateDatetime"
													format="yyyy-MM-dd HH:mm:ss" />
											</td>
											<td valign="middle">${monitorCreateUsername}</td>
											<td valign="middle"><c:out value="${monitorMemo}"></c:out></td>
										</tr>
									</s:iterator>
									</tbody>
									<tr>
