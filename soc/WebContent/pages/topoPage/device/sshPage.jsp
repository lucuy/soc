...
<script type="text/javascript">
	...
	function sshConnect() {
		...
		//alert("123");
			var url = "${ctx}/deviceCategory/sshConnect.action";
/*			$.post(url, function(data) {
			...
			$.ajax({
			      type : "post",
				   url: url,
				   data:{deviceIp:deviceIp,devicePort:devicePort,username:username,password:password},
	...
	function disconnect(){
		var url = "${ctx}/deviceCategory/sshDisConnect.action";
		$.ajax({
			type:"post",
			url:url,
			success:function(result){
	...
	function doCmd(){
		...
		var url = "${ctx}/deviceCategory/doSshCmd.action";
...
<%-- <input type="text" name="deviceIp" id="deviceIp"  value="${deviceIp }"/>
		...
		&nbsp;&nbsp;
				登录名:
				 ...
				 密码:
		 		 ...
		 		 <input type="button"  name="connectPort" id="connectPort" value="连接" style="width: 100px;height: 30px " onclick="sshConnect();"/>
		...
		<div style="color: #fff; margin-top: 8px;">
				命令:
		 		...
		 		<input type="button"  name="doCmd" id="doCmd" value="执行" style="width: 100px;height: 30px " onclick="doCmd();"/>
		 </div>
		 <!-- <table>
		 	<tr>
		 		<td align="left">
		 	...
		 	<tr>
		 	<td align="left">
		 			 <div style="color: #fff">username:
				 <input type="text" name="username" id="username"  style="height: 25px;"/></div>
				</td>
				<td align="left">
					<div style="color: #fff"> password:
		 			 <input type="password" name="password" id="password" style="height: 25px;"/></div>
				</td>
		 	<td align="left">
					 <input type="button"  name="connectPort" id="connectPort" value="连接" onclick="sshConnect();"/>
				</td>
				<td align="left">
