...
<script language="javascript" >
	...
	function updateStatus() {
		...
		if ($(":checkbox[checked = true]").size() < 1) {
			$('#dialog-inUserSave').dialog('open').text("请至少选择一个用户信息...");
				// 内置用户弹框			
				$('#dialog-inUserSave').dialog({
					buttons : {
						"确定[Enter]" : function() {
		...
		if ($("#status").val() == 100) {
			$('#dialog-inUserSave').dialog('open').text("请选择需要执行的操作...");
				// 内置用户弹框			
				$('#dialog-inUserSave').dialog({
					buttons : {
						"确定[Enter]" : function() {
		...
		//内置用户判断
		if(c==43||c==41||c==42){
			$('#dialog-inUserSave').dialog('open').text("包含系统内置用户，不能锁定");
				// 内置用户弹框			
				$('#dialog-inUserSave').dialog({
					buttons : {
						"确定[Enter]" : function() {
		...
		if (1==1) {
			$('#dialog-inUserSave').dialog('open').text("确认执行操作吗？");
				// 内置用户弹框			
				$('#dialog-inUserSave').dialog({
					buttons : {
						"确定[Enter]" : function() {
	...
	function inUserSave(userId){
			if(userId==43||userId==41||userId==42){
				$('#dialog-inUserSave').dialog('open');
				// 内置用户弹框			
				$('#dialog-inUserSave').dialog({
					buttons : {
						"确定[Enter]" : function() {
							location.href="${ctx}/user/editUser.action?userId="+userId+"&falg=0";
						}
					}
				});
				}else{
	...
	function deleteUser() {
		...
		if(c==43||c==41||c==42){
		      $('#dialog-inUserSave').dialog('open').text("包含系统内置用户，不能删除");
				// 内置用户弹框			
				$('#dialog-inUserSave').dialog({
					buttons : {
						"确定[Enter]" : function() {
		...
		for ( var i = 0; i < idss.length; i++) {
			if(idss[i]==myid){
				 $('#dialog-inUserSave').dialog('open').text("不能删除当前登陆用户，请重新选择");
				// 内置用户弹框			
				$('#dialog-inUserSave').dialog({
					buttons : {
						"确定[Enter]" : function() {
...
<body style="margin-top:2px;">
			...
			<tr>
				<td>
					<div class="box">
						...
						<div class="right">
					   <!--  <input type="button" value="同步" class="btnstyle"
								style="margin-right:5px;margin-top:-2px"
								onclick="accountAjax();" />  -->
								
							<input type="button" value="添加" class="btnstyle"
								style="margin-right:5px;margin-top:-2px"
								onclick="location.href='${ctx}/user/editUser.action?userStatus=${userStatus}&order=${order}&falg=1';" />
									...
									<tbody>
										...
										<s:iterator value="userList" status="stat">
											...
											<tr>
												...
												<td valign="middle"><a
													href="#" id="${userId}" onclick="inUserSave(this.id);">${userLoginName}&nbsp;&nbsp;(${userRealName})</a>
												</td>
												<td valign="middle"><c:forEach items="${userRoleList}"
														var="l">${l.roleName}&nbsp;</c:forEach>
												</td>
												<td valign="middle" align="center">
												<s:iterator value="allAssetGroupList" status="stat">
												<c:if test="${assetGroupid eq assetGroupId}">${assetGroupName}</c:if>
												</s:iterator>
												
												</td>
												<td valign="middle" align="center"><c:if
														test="${userStatus==1}">激活</c:if> <c:if
														test="${userStatus==0}">锁定</c:if>
												</td>
												<c:set value="0" var="root2" />
												<c:set value="" var="resStr" />

												<td valign="middle"><a href="mailto:${userEmail}">${userEmail}</a>
												</td>
												<td valign="middle">${userMobile}</td>
											</tr>
	...
	<!-- ui-dialog -->
	<div id="dialog-inUserSave" title="系统提示">
		<table width="90%" border="0" cellspacing="5" cellpadding="5"
			style="margin-left:20px;">
			<tr><td>内置用户不能修改  </td></tr>
		</table>
	</div>
