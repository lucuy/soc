...
<script language="javascript">
	...
	function query() {
		...
		location.href = "${ctx}/filteringRules/queryFilterRuleList.action?keyword="
	...
	function deleteSelect() {
		...
		if (confirm("确认执行操作吗？")) {
			...
			//alert($("#status").val());
			location.href = "${ctx}/filteringRules/deleteFilterRule.action?ids="
					+ ids;
		}
	...
	function extQuery() {
		var str=$('#filteringRuleNamevalue').val().match(/^([^`~!@#$%^&*()=|{}':;, \\\[\].<>\/?~！@#￥……&*（）—|{}【】‘；：”“'。\"，、？])*$/); 
		if(str==null){
			alert("过滤规则名称包含特殊字符！");
			return;
		}
		$('#filteringRuleName').val($('#filteringRuleNamevalue').val());
		$('#filteringRuleType').val($('#filteringRuleTypevalue').val());
	...
	function updateStatus() {
		...
		if (confirm("确认执行操作吗？")) {
			location.href = "${ctx}/filteringRules/updateStatus.action?ids="
					+ ids + "&status=" + $("#status").val();
		}
	}
	function toInsertPage() {
		//alert("{ctx}/filteringRules/toInsertFilterPage.action");
		location.href = "${ctx}/filteringRules/toInsertFilterPage.action";

	}
</script>
</head>
<body>
	<s:form action="queryFilterRuleList" namespace="/filteringRules"
		method="post" theme="simple" id="sysForm" name="sysForm">
		<input type="hidden" name="filteringRuleName"
			value="${filteringRuleName}" />
		<input type="hidden" name="filteringRuleType"
			value="${filteringRuleType}" />
				...
				<td>
					<table style="margin-top: 4px" width="100%" border="0"
						...
						<tr height="28" class="biaoti"> 
							...
							<th width="16%" align="center" class="biaoti">过滤规则名称</th>
						...
						<s:iterator value="list">
							<tr>
								<td class="biaocm" valign="middle"><input type="checkbox"
									name="ids" id="${filteringRuleId}" value="${filteringRuleId}"
									class="check-box" /></td>
								<td valign="middle" class="td_list_data"><a
									href="selectFilterRuleById.action?id=${filteringRuleId}"
									title="${filteringRuleName }">${filteringRuleName}</a></td>
								<td valign="middle" class="td_list_data"><c:if
										test="${filteringRuleType==1}">停用</c:if> <c:if
										test="${filteringRuleType==0}">启用</c:if></td>
								<td valign="middle" class="td_list_data"><s:date
										name="filteringRuleMenderDate" format="yyyy-MM-dd HH:mm:ss" />
								</td>
							</tr>
						</s:iterator>
						</tbody>
	...
	<s:form action="queryFilterRuleList" namespace="/filteringRules"
		...
		<s:hidden id="filteringRuleName" name="filteringRuleName" />
		<input type="hidden" id="filteringRuleType" name="filteringRuleType" />
	</s:form>
	...
	<div id="dialog-extQuery" title="高级查询">
		<table width="90%" border="0" cellspacing="5" cellpadding="5"
			...
			<tr>
				<td width="25%">过滤规则名称:</td>
				<td><input id="filteringRuleNamevalue"
					name="filteringRuleNamevalue" type="text" style="width:250px;"
					onkeypress="if(event.keyCode==13)extQuery();" />
				</td>
			</tr>
			<tr>
				<td>启用状态:</td>
				<td><select id="filteringRuleTypevalue"
					name="filteringRuleTypevalue">

						<option value="" style="margin-right:24px">--请选择过滤规则状态--</option>
