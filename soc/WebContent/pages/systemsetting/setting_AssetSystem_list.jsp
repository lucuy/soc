...
<script language="javascript" >
	...
	function query() {
		...
		location.href = "${ctx}/assetSystem/queryAssetSystem.action?keyword="
	...
	function deleteUser() {
		...
		if (confirm("确认执行操作吗？")) {
			location.href = "${ctx}/assetSystem/deleteAssetSystem.action?ids=" + ids;
		}
	}
		function clearFile() {
...
<body style="margin-top:2px;">
	<s:form action="queryAssetSystem" namespace="/assetSystem" method="post"
		theme="simple" id="assetSystemForm" name="assetSystemForm">
					...
					<div class="box">
						...
						<div class="right">
							...
							<input type="button" value="添加" class="btnstyle"
								...
								onclick="location.href='${ctx}/assetSystem/toEditAssetSystem.action';" />
						...
						<div class="cont">
							...
							<div id="dataList">
								<table width="100%" border="0" cellspacing="1" cellpadding="0"
									...
									<tbody>
										<s:iterator value="systemlist" status="stat">
											<tr>
												<td class="biaocm" valign="middle"><input
													type="checkbox" name="ids" id="${assetSystemId}" value="${assetSystemId}"
													class="check-box" />
												</td>
												<td valign="middle"><a
													href="${ctx}/assetSystem/toEditAssetSystem.action?asId=${assetSystemId}">${assetSystemName} </a>
												</td>
												
												<c:set var="upsName" value="" />
												<s:iterator value="brandlist">
													<c:set var="upsName" value="${upsName}[${assetSystemName}]<br/>" />
												</s:iterator>
												<td valign="middle" class="td_list_data italic hand" align="center"
													onmouseover="overDlg(this,'系统品牌列表','${upsName}','');" onmouseout="destory();">
													系统品牌列表
												</td>
												<c:set var="upsName" value="" />
												...
												<td valign="middle" class="td_list_data italic hand" align="center"
													onmouseover="overDlg(this,'系统版本列表','${upsName}','');" onmouseout="destory();">
													系统版本列表
												</td>
												<c:set var="upsName" value="" />
												<s:iterator value="versionnolist">
													<c:set var="upsName" value="${upsName}[${assetSystemName}]<br/>" />
												</s:iterator>
												<td valign="middle" class="td_list_data italic hand" align="center"
													onmouseover="overDlg(this,'系统版本号列表','${upsName}','');" onmouseout="destory();">
													系统版本号列表
												</td>
											</tr>
										</s:iterator>
