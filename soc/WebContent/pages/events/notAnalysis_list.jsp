...
var rege =/^([^`~!@#$%^&*()=|{}':;, \\\[\].<>\/?~！@#￥……&*（）—|{}【】‘；：”“'。\"，、？])*$/;
	...
	function extQuery() {
		 ...
		 t=new Date().format("yyyy-MM-dd"); 
			...
			$("#msgs").append(html);
			//验证字符串长度
			if($('#notAnalyticEventsAssetName').val().length>50){
				alert("资产名长度不能大于50");
				$('#notAnalyticEventsAssetName').val('');
				$('#notAnalyticEventsAssetName').focus();
				return ;
			}
			//验证是否包含特殊字符
			if(!rege.test($('#notAnalyticEventsAssetName').val())){
				alert("输入的内容包含特殊字符，请重新输入");
				$('#notAnalyticEventsAssetName').val('');
				$('#notAnalyticEventsAssetName').focus();
				return;
			...
			//验证字符串长度
			if($('#notAnalyticEventsCollectorName').val().length>50){
				alert("采集器长度不能大于50");
				$('#notAnalyticEventsCollectorName').val('');
				$('#notAnalyticEventsCollectorName').focus();
				return ;
			}
			//验证是否包含特殊字符
			if(!rege.test($('#notAnalyticEventsCollectorName').val())){
				alert("输入的内容包含特殊字符，请重新输入");
				$('#notAnalyticEventsCollectorName').val('');
				$('#notAnalyticEventsCollectorName').focus();
				return;
			...
			//验证字符串长度
			if($('#notAnalyticEventsContent').val().length>50){
				alert("事件详情长度不能大于50");
				$('#notAnalyticEventsContent').val('');
				$('#notAnalyticEventsContent').focus();
				return ;
			}
		...
		$('#selnotAnalyticEventsContent').val($.trim($('#notAnalyticEventsContent').val()));
		$('#selbeginTime').val($.trim($('#beginTime').val()));
		$('#selendTime').val($.trim($('#endTime').val()));
		$('#selday').val($.trim($('#day').val()));
		if(flag==true){
			location.href="${ctx}/notAnalyticEvents/queryAllNotAnaly.action?selday="+$('#selday').val()+"&selendTime="+$('#selendTime').val()+"&selbeginTime="+$('#selbeginTime').val()
					+"&selnotAnalyticEventsAssetName="+$('#selnotAnalyticEventsAssetName').val()+"&selnotAnalyticEventsContent="+$('#selnotAnalyticEventsContent').val()
					+"&selnotAnalyticEventsCollectorName="+$('#selnotAnalyticEventsCollectorName').val()
					;
				...
				<td id="mes">
					<div class="box">
						...
						<span class="kuaiju">快速搜索</span> <input type="text" id="keyword"
							...
							onclick="extQueryDlg();">高级</a> <input type="button" value="打印" style="margin-left:5px;float:right;margin:4.5px 5px;" class="btnstyle" onclick="preview();">
					</div>
				</td>
			<tr>
				<td>
				<div id="dataList">
					<table style="margin-top: 4px" width="100%" border="0"
						...
						<tr height="28" class="biaoti">
							<th width="10%" align="center" class="biaoti">事件编号</th>
							<th width="10%" align="center" class="biaoti">关联资产</th>
							<th width="10%" align="center" class="biaoti">关联采集器</th>
						...
						<tr >
							<td colspan="4" width="100%" ><div id="over"><jsp:include
		...
		<table width="90%" border="0" cellspacing="5" cellpadding="5"
			...
			<tr>
				...
				<td ><span class="display" id="custom"> <input value = ""
										...
										href="javascript:custom('return'); " style="color: blue;">返回</a> 
										<br/><font color="red"><span id="msgs"></span></font>
										</span> 
...
<a
										href="javascript:custom('custom');" style="color: blue;">自定义</a>
								</span>
								</td>
							</tr>
		</table>
