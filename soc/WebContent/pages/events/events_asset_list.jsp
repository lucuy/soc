...
var rege =/^([^`~!@#$%^&*()=|{}':;, \\\[\].<>\/?~！@#￥……&*（）—|{}【】‘；：”“'。\"，、？])*$/;
	...
	function query() {
		...
		location.href = "${ctx}/notAnalyticEvents/queryAssetOfEvents.action?keyword="
...
<body>
	<s:form action="queryAssetOfEvents"
			...
			<tr>
				<td>
					<div class="box">
						...
						<span class="kuaiju">查看
						
						
							<select id="day" name="day">
								<option value="1" <c:if test="${day == 1}">selected="selected"</c:if>>1天</option>
								<option value="2" <c:if test="${day == 2}">selected="selected"</c:if>>2天</option>
								<option value="3" <c:if test="${day == 3}">selected="selected"</c:if>>3天</option>
								<option value="4" <c:if test="${day == 4}">selected="selected"</c:if>>4天</option>
								<option value="5" <c:if test="${day == 5}">selected="selected"</c:if>>5天</option>
								<option value="6" <c:if test="${day == 6}">selected="selected"</c:if>>6天</option>
								<option value="7" <c:if test="${day == 7}">selected="selected"</c:if>>7天</option>
							</select>
						
							内的未采集事件的资产&nbsp;&nbsp;
							<input type="button" value="执行" style="margin-left:5px" class="btnstyle" onclick="query();">
				...
				<td>
					<table style="margin-top: 4px" width="100%" border="0"
						...
						<tr height="28" class="biaoti">
							<th width="17%" align="center" class="biaoti">资产</th>
							<th width="17%" align="center" class="biaoti">ip地址</th>
							<th width="17%" align="center" class="biaoti">所属组</th>
							<th width="17%" align="center" class="biaoti">重要性</th>
							<th width="26%" align="center" class="biaoti">状态</th>
						</tr>
                        </thead>

                        <tbody>
							<s:iterator value="#request.assetList" status="stat">
								...
								<tr>
									...
									<td valign="middle" align="center">
										<c:if test="${assetStatus==1}">启用</c:if>
										<c:if test="${assetStatus==0}">停用</c:if>
									</td>
								</tr>
							</s:iterator>
                        </tbody>
