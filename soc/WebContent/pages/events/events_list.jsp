...
<script language="javascript">
	...
	function exportExcel() {
		var ids = "";
		  var temp = $("#status").val();
	        ...
	        var ip =$("#ip").val();
	        var linkType =$("#queryEventsType").val();
	        var queryEventsId=$("#queryEventsId").val();
		...
		if(ids == ""){
			if(confirm("你是要导出所有事件信息..")){
				location.href = "${ctx}/events/exportEventExcel.action?queryEventsId="+queryEventsId+"&queryEventsType="+linkType+"&eventsType="+eventCategory+"&ip="+ip+"&status="+temp;
			}
		}else{
			
				if (confirm("你是要导出选中的事件信息...")) {
					location.href = "/soc/events/exportEventExcel.action?checkids="+ ids+"&queryEventsId="+queryEventsId;
	...
	//document.getElementById("key").value=encodeURI(encodeURI("资产风险评估图", "utf-8"));
		...
		if(ids == ""){
			if(confirm("你是要导出所有事件信息..")){
				location.href = "${ctx}/events/exportEvent.action?queryEventsId="+queryEventsId+"&queryEventsType="+linkType
						+"&eventsType="+eventCategory+"&ip="+ip+"&status="+temp+"&reportType="+reportType+"&eventReportId=12";
			}
		}else{
			
				if (confirm("你是要导出选中的事件信息...")) {
					location.href = "/soc/events/exportEvent.action?checkids="+ ids
							+"&queryEventsId="+queryEventsId+"&reportType="+reportType+"&eventReportId=12";
...
function del_blank(str)
{
    ...
    //定时更新数据
    function reFresh(queryEventsType)
    {
        sourceType = queryEventsType;  //记录查询条件类型
        if(queryEventsType == 0)
        {
            if( m >= 1)
            {
                $("#mes").html("<div class='box'><div id='refresh' class='refresh'><b>"+m+"</b> 秒后更新数据</div></div>");
                refresh = setTimeout("reFresh("+queryEventsType+")",1000);
                m--;
            }
            else
            {
     ...
     function changeIcon(nainNode)
    {
     ...
     function selectnormal()
     {
        ...
        if(ip=='')
        {
           location.href='${ctx}/events/queryEvents.action?queryEventsId='+queryEventsId+'&queryEventsType='+linkType+'&eventsType='+eventCategory+'&ip='+ip+'&status='+temp;
        } 
        else
        {
              if(regex.test(ip)) {
...
<body>
	...
	<input type="hidden" id="key"/>
		...
		<s:form action="queryEvents.action" namespace="/events" method="post"
			theme="simple" id="events" name="eventsForm">
			<input type="hidden" name="eventsType" value="${eventsType}"/>
			<input type="hidden" name="deviceIp" value="${deviceIp}"/>
			<input type="hidden" name="sourceAdd" value="${sourceAdd}"/>
			<input type="hidden" name="targetAdd" value="${targetAdd}"/>
...
<div class="box"> 
	<div  style = "margin-bottom:0;">
                   ...
                   <option value="" selected="selected">---请选择事件级别---</option>
                    <option value="1" <c:if test="${eventsType =='1' }">selected="selected"</c:if>>一级 </option>
				   
                    <option value="2"<c:if test="${eventsType == '2' }">selected="selected"</c:if>>二级</option>
                   
                    <option value="3"<c:if test="${eventsType == '3' }">selected="selected"</c:if>>三级</option>
                    <option value="4"<c:if test="${eventsType == '4' }">selected="selected"</c:if>>四级</option>
                   
                    <option value="5"<c:if test="${eventsType == '5' }">selected="selected"</c:if>>五级</option>
                   </select>
                   </span>
                   ...
                   <input type="button" value="执行" style="margin-left:5px"
					...
					<div class="right" style="margin-top: 0px">
							<input type="button" value="导出" class="btnstyle"
								 align="right" style="margin-left:400px"
										...
										<tbody>
											...
											<s:iterator value="eventsList">
												<tr class="back" id="eventsTr_${eventsLogId}_${eventsType}"
													...
													ondblclick="dblclick('${eventsLogIdentification}','${eventsType}','${relLogTableName}','${eventsCustomd1}');">
													<td valign="middle" class="td_list_data">
														<input type="checkbox" id="ids" name="ids" value="${eventsId}" class="check-box"/>
													</td>
													<td valign="middle" align="center">${num+1}</td>
											<td  >
														<div class="level" style = "margin:0px auto;">
															<c:if test="${eventsLevel <= 1}">
																<div class="levelBa"
																	style="background-color:#CCCCCC; width:${eventsLevel*8}px;">
																</div>
																<span style="position:relative;left:0px;top:-20px;">${eventsLevel}</span>
															</c:if>
															<c:if test="${eventsLevel > 1 && eventsLevel <= 3}">
																<div class="levelBa"
																	style="background-color:#ffcc33;  width:${eventsLevel*8}px;">
																</div>
																<span style="position:relative;left:0px;top:-20px;">${eventsLevel}</span>
															</c:if>
															<c:if test="${eventsLevel >= 4}">
																<div class="levelBa"
																	style="background-color:#ff3333; width:${eventsLevel*8}px;">
																</div>
																<span style="position:relative;left:0px;top:-20px;">${eventsLevel}</span>
															</c:if>
														</div></td>
													<td valign="middle" align="center">${eventsNameCustomd}</td>



													<td valign="middle" align="center">${eventsTypeCustomd}</td>
													
													
													
													<td valign="middle" align="center">${eventsDevName}</td>
													
													
													
													<td valign="middle" align="center">
														<s:date name="eventsTime" format="yyyy-MM-dd HH:mm:ss" />
													</td>

													<td valign="middle" align="center">${eventsSourceAddT}</td>

													<td valign="middle" align="center">${eventsTargetAddT}</td>
													
													<td valign="middle" align="center">${eventsSourcePort}</td>
											...
											<tr style="background-color:#FFFFFF; ">
												<td colspan="12" width="100%"><jsp:include
	...
	<div id="dialog-enentsContent" title="详细信息"
		...
		<div class="column">
			<div class="title">
				<span class="title_t"><img
					src="${ctx}/images/u319_normal.gif" id="img_5" class="img_a"
		...
		<div class="column">
			<div class="title">
				<span class="title_t"><img
					src="${ctx}/images/u319_normal.gif" id="img_7" class="img_a"
	...
	<%-- <s:form name="exportEvent" namespace="/events" action="exportEvent" id="exportEvent" method="post" theme="simple">
		...
		<input type="hidden"  name="eventReportId" id="alertReportId" value = "12"/>  
