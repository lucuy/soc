...
$(document).ready(function() {
	...
	$.getJSON("${ctx}/demandCollet/queryTree.action?dataTime="+new Date(), function(result) {
		var contentStr = "";
		if(result.length == 0){
			parent.mainFrame.location.href="${ctx}/pages/dbPage/cpManage/demand/rectificationProposal/proposalList.jsp?data=nodata";
		}else{
			parent.mainFrame.location.href="${ctx}/proposal/queryProposal.action?CIA_AssessResult=1&FK_CA="+  result[0].id+"&sysname="+result[0].name;
		}
		$.each(result, function(i, item) {
			contentStr+="<li><a href=\"javascript:void(0);\">"+item.name+"</a>";
			contentStr+="<ul>";
			contentStr+=" <li><a href=\"${ctx}/proposal/queryProposal.action?FK_CA="+item.id+"&sysname="+item.name+"&CIA_AssessResult=1\" target=\"mainFrame\"  title=\"部分符合项\">部分符合项</a></li>";
			contentStr+=" <li><a href=\"${ctx}/proposal/queryProposal.action?FK_CA="+item.id+"&sysname="+item.name+"&CIA_AssessResult=2\" target=\"mainFrame\" title=\"不符合项\">不符合项</a></li>";
			contentStr+="</ul>";
			contentStr+="</li>";
		});
...
<table width="100%"  cellSpacing=0 cellPadding=0  >
  <tbody>
	...
	<td class="leftDhxx">整改建议汇总&nbsp;&nbsp;<a href="javascript:linkTo('rank');">返回</a></td>
    </tr>
    <tr>
    <tr>
      <td></td>
