...
$.getJSON("${ctx}/msaShow/queryTree.action?dataTime="+new Date(), function(result) {
	var contentStr = "";
	 if(result.processLog[0]==null){
	 		parent.mainFrame.location.href="${ctx}/msaRect/msaRectList.action?";
		}else{
			parent.mainFrame.location.href="${ctx}/msaRect/msaRectList.action?type=1&msaDate="+result.processLog[0].msaDate;
		}
	$.each(result.processLog, function(i, item) {
		
		contentStr+="<li><a href=\"javascript:void(0);\">"+item.msaDate+"</a>";
		contentStr+="<ul>";
		contentStr+=" <li><a href=\"${ctx}/msaRect/msaRectList.action?msaDate="+item.msaDate+"&type=1\" target=\"mainFrame\"  title=\"部分符合项\">部分符合项</a></li>";
		contentStr+=" <li><a href=\"${ctx}/msaRect/msaRectList.action?msaDate="+item.msaDate+"&type=2\" target=\"mainFrame\" title=\"不符合项\">不符合项</a></li>";
		contentStr+=" <li><a href=\"${ctx}/msaRect/msaRectList.action?msaDate="+item.msaDate+"&type=3\" target=\"mainFrame\" title=\"不适用\">不适用</a></li>";
		contentStr+="</ul>";
		contentStr+="</li>";
