...
$.getJSON("${ctx}/msaShow/queryTree.action?dataTime="+new Date(), function(result) {
	...
	$.each(result.processLog, function(i, item) {
		...
		contentStr+="<ul>";
		contentStr+=" <li><a href=\"${ctx}/msaShow/msaShowNoFit.action?msaDate="+item.msaDate+"&type=1\" target=\"mainFrame\"  title=\"部分符合项\">部分符合项</a></li>";
		contentStr+=" <li><a href=\"${ctx}/msaShow/msaShowNoFit.action?msaDate="+item.msaDate+"&type=2\" target=\"mainFrame\" title=\"不符合项\">不符合项</a></li>";
		contentStr+=" <li><a href=\"${ctx}/msaShow/msaShowNoFit.action?msaDate="+item.msaDate+"&type=3\" target=\"mainFrame\" title=\"不适用\">不适用</a></li>";
		contentStr+="</ul>";
		contentStr+="</li>";
	});
