...
$(document).ready(function() {
		var gpaDateFirst=$.trim($("#gpaDateFirst").val());
		var typeFirst=$.trim($("#typeFirst").val());
		if(gpaDateFirst!=""){
		   document.getElementById("gpaDate").value=gpaDateFirst;
		   document.getElementById("type").value=typeFirst;
		   document.getElementById("gpaDateFirst").value="";
...
function goSearch(num)
  {
	 var gpaDate=$.trim($("#gpaDate").val());
	  var type=$.trim($("#type").val());
	var keyword = $.trim($("#goToSearch").val());
	 if(keyword != ''&& gpaDate!=''){
		jsonForAjax("${ctx}/gpaShow/queryAjaxGpaShowNoFit.action?startIndex="+encodeURI(num,"utf-8")+ "&keyword=" + encodeURI(encodeURI(keyword,"utf-8"))+ "&gpaDate=" + encodeURI(gpaDate,"utf-8")+ "&type=" + encodeURI(encodeURI(type,"utf-8"))+"&data="+new Date );
	 }else if(keyword == ''&& gpaDate!=''){
		jsonForAjax("${ctx}/gpaShow/queryNotKeyAjaxGpaShowNoFit.action?startIndex="+encodeURI(num,"utf-8")+ "&gpaDate=" + encodeURI(gpaDate,"utf-8")+ "&type=" + encodeURI(encodeURI(type,"utf-8"))+"&data="+new Date );
	} 
  };
  ...
  function goToPage(lastnum) {
	...
	function jsonForAjax(url) {
		...
		$.getJSON(url,function(result) {
							...
							$.each(result.gpaShows,function(i, item) {
...
<body style="margin: 0;position: 0;padding-left:2px">
					...
					<div class="sbox">
						<div class="cont">
							...
							<div id="dataList">
								...
								<thead>
									<tr >				 
										<th width="20%" class="biaoti" onclick="sortAble('table-userManage', 0)"  style="cursor:pointer" >控制域</th>
    ...
    <input type="hidden" name="gpaDate" id="gpaDate"  value="<s:property value="#parameters.gpaDate"/>">
    <input type="hidden" name="type" id="type" value="<s:property value="#parameters.type"/>">
    <input type="hidden" id="gpaDateFirst"  value="<s:property value="#request.gpaDateFirst"/>">
