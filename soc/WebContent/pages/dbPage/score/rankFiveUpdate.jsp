...
<link href="${ctx}/styles/dbStyles/template.css" rel="stylesheet" type="text/css">
 ...
 function ifShowDocDiv(docu){
      if(docu.value=="1"){
       ...
       document.getElementsByName("rankDoc")[1].disabled="disabled";
 }
	 ...
	 if(isDeps[0].checked){
		 ...
		 if(!(depJudges[0].checked) && !(depJudges[1].checked)){
	       ...
	       document.scoreresultForm.action="";
			...
			formName.action = "${ctx}/rank/rankAction_rankFiveUpdate.action";
			formName.submit();
 ...
 function addFiles(){
		...
		fileInput.type="file";
		fileInput.name="frankAccess";
		fileInput.id="frankAccess";
		var upButton=document.createElement("input");
		upButton.type="button";
		upButton.value="上传文件";
		
		upButton.onclick=function upfile(){
			var frankAccess=document.getElementById("frankAccess");
		...
		filesDiv.removeChild(upbu);
		 var upbuh=document.getElementById("frankAccessFileNamehidden");
...
<input type="hidden" name="rankCount" value="<s:property value="rank.rankCount"/>" />
    ...
    <table  width=100% border="1" bordercolor="#5AA4D1" cellpadding="0" cellspacing="0"  >
	    ...
	    <tr>	
		    ...
		    <td >
		     <input type="text" name="rankGrade" value="<s:property value="rank.rankGrade"/>" id="rankGrade" value="<s:property value="rank.rankGrade"/>" readonly="readonly" style="width: 200px"/>
		    </td>
		    <td align="right"><font color="#ff0000">*</font>02定级时间</td>
		    <td  ><input type="text" name="rankTime" id="rankTime" value="<s:property value="rank.rankTime"/>" style="width:200px" readonly="readonly"  onclick="SelectDate(this)" ></td>
	    ...
	    <tr>
		    ...
		    <td align="right">
			 04是否有主管部门			  </td>
		    <td >
			    <input type="radio" name="rankIsDep" onclick="hasDep()"   value="1" <s:if test="%{1==rank.rankIsDep}">checked</s:if> > 有
			    <input type="radio" name="rankIsDep" value="0" onclick="hasDep()"   <s:if test="%{0==rank.rankIsDep}">checked</s:if> >无（如选择有请填下两项）			  </td>
	    </tr>
	    <tr>
		    <td align="right"><font color="#ff0000">*</font>05主管部门名称</td>
		    <td >				
			    <input type="text" name="rankDepName" maxlength="50" value="<s:property value="rank.rankDepName"/>" id="rankDepName" style="width: 200px" class="txt 100" onblur="yanzheng1(this)">			  </td>
		    <td align="right"><font color="#ff0000">*</font>06主管部门审批定级情况</td>
		    <td >				
				    <input type="radio" name="rankDepJudge" value="1"  <s:if test="%{1==rank.rankDepJudge}">checked</s:if> >已审批
				    <input type="radio" name="rankDepJudge" value="0"  <s:if test="%{0==rank.rankDepJudge}">checked</s:if> >未审批			  </td>
	    </tr>
	    <tr>
		    <td align="right">07系统定级报告</td>
		    <td >
			    <input type="radio" name="rankDoc" onclick="ifShowDocDiv(this)"  value="1" <s:if test="%{1==rank.rankDoc}">checked</s:if> >有
			    <input type="radio" name="rankDoc"  value="0" <s:if test="%{0==rank.rankDoc}">checked</s:if> >无			  
			</td>
		    <td align="right">定级报告</td>	
		    <td >
		    <div id="divUploadify_main"> 
		    <input type="hidden" name="frankAccessFileName" maxlength="200" id="frankAccessFileNamehidden" value="<s:property value="rank.rankAccess"/>">      
      <input type="button" value="替换附件" onclick="addFiles();" name="updatebutton" id="updatebutton"  <s:if test="%{0==rank.rankDoc}">disabled</s:if>  >
    </div>	<span id="msg"></span>		
			  <%-- <input type="text" name="rankAccess"    id="rankAccess" value="<s:property value="rank.rankAccess"/>" style="width: 200px"> --%>			  
			 </td>	
	    </tr>
	    <tr>
		    <td align="right"><font color="#ff0000">*</font>填表人</td>
		    <td ><input type="text" name="rankInformant"  maxlength="50" value="<s:property value="rank.rankInformant"/>" id="rankInformant" style="width: 200px" onblur="yanzheng1(this)"></td>
		    <td align="right"><font color="#ff0000">*</font>填表日期</td>
		    <td ><input type="text" name="rankDate" value="<s:property value="rank.rankDate"/>" readonly="readonly"  onclick="SelectDate(this)" id="rankDate" style="width: 200px"  class="txtRequired 100"></td>
	    </tr>
    </table>
