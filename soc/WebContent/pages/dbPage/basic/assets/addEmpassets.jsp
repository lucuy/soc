...
function back()
{
	location.href="${ctx}/pages/dbPage/basic/assets/empassets.jsp";
}
...
function save(){
  ...
  //验证姓名，只能输入字母、汉字、字母汉字组合，禁止输入特殊字符 否则则清空
      function ChooseEmpName()
      {
          var reg=/^([\u0391-\uFFE5A-Za-z])+$/;
          if(reg.test($.trim($("#empName").val())) || $.trim($("#empName").val())=="")
          {
            return true;
          }else
          {
             alert("输入字符不符合,只能输入字母、汉字、字母汉字组合，禁止输入特殊字符");
             document.getElementById("empName").value="";
          }
      }
      
        //验证联系方式只能输入字母、汉字、字母汉字组合，禁止输入特殊字符 否则则清空
      function ChooseConInfo()
      {
          /*var reg=/^(?:[\u4e00-\u9fa5]*\w*\s*)+$/;
          if(reg.test($.trim($("#conInfo").val())) || $.trim($("#conInfo").val())=="")
          {
            return true;
          }else
          {
             alert("输入字符不符合,只能输入字母、汉字、字母汉字组合，禁止输入特殊字符");
             document.getElementById("conInfo").value="";
          }
          */
    	  var reg1 = /^(\d{3,4}\-?)?\d{7,8}$/;
    	  var reg2 = /^0?(13[0-9]|15[012356789]|18[0236789]|14[57])[0-9]{8}$/;
    	  if(reg1.test($.trim($("#conInfo").val())) || $.trim($("#conInfo").val())=="" || reg2.test($.trim($("#conInfo").val())))
          {
            return true;
          }else
          {
             alert("联系方式不合法，请输入正确的联系方式！");
             document.getElementById("conInfo").value="";
          }
    ...
    return this.length + (cArr == null ? 0 : cArr.length);       
     ...
     function textLimitCheck(thisArea, maxLength){    
       ...
       if(len <= maxLength)
        {
         if(ma.test($.trim($("#jobDes").val())) || $.trim($("#jobDes").val())=="")
          {
            return true;
           }else
            {
   ...
   if(len <= maxLength)
   {
     if(main.test($.trim($("#empRemarks").val())) || $.trim($("#empRemarks").val())=="")
     {
         return true;
      }else
      {
  ...
  <s:form action="add" method="post" namespace="/empAssets" theme="simple" id="empAssets">
  <input type="hidden" name="id" value="${empAssets.id}">
				...
				<td width="100%" valign="top">
					...
					<div class="framDiv">
						<table width="100%" border="0" cellspacing="1" cellpadding="0">
							<tr>
								<td colspan="4" class='r2titler' style="font-size: 12px">安全相关人员编辑</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td style="text-align:left;">安全相关人员
								</td>
							</tr>
							<tr>
								<td class="td_detail_separator"></td>
							...
							<tr>
								<td class="tdleft">所管理系统名称：<font color="#ff0000">*</font></td>
								<td style="text-align:left">
									<input type="text" style="width:250px" id="sysName" width="99%" name="sysName" maxlength="500" value="${empAssets.sysName}" readonly="readonly" onclick="$('#goToSearch').val('');employeeDlg()" />
								</td>
							</tr>
							<tr>
								<td class="td_detail_separator"></td>
							...
							<tr>
								<td class="tdleft">姓&nbsp;名：<font color="#ff0000">*</font></td>
								<td style="text-align:left">
									<input type="text" id="empName"  name="empName" maxlength="50" value="${empAssets.empName}" style="width:250px" onblur="ChooseEmpName(this.value)" />
								</td>
							</tr>
							<tr>
								<td class="td_detail_separator"></td>
							...
							<tr>
								<td  class="tdleft">联系方式：<font color="#ff0000">*</font></td>
								<td style="text-align:left">
									<input type="text" style="width:250px" id="conInfo" maxlength="50" name="conInfo"  onblur="ChooseConInfo(this.value)" value="${empAssets.conInfo}"/>
								</td>
							</tr>
							...
							<tr>
								<td class="tdleft">岗位描述：&nbsp;</td>
								<td colspan="3" style="text-align: left;">
									<textarea rows="3" style="width:40%" id="jobDes" maxlength="50" name="jobDes">${empAssets.jobDes}</textarea>
								</td>
							</tr>
							<tr>
								<td class="td_detail_separator"></td>
							...
							<tr>
								...
								<td colspan="3" style="text-align: left;">
									<textarea rows="3" style="width:40%" id="empRemarks" maxlength="50" name="empRemarks">${empAssets.empRemarks}</textarea>&nbsp;
								</td>
							</tr>
							<tr>
								<td class="td_detail_separator"></td>
...
<!-- ui-dialog-employee -->
		<div id="dialog-employee" title="所管理信息系统" style="font-size: 12px;">
			...
			<tr>
				<td>
					...
					<tr height="28">
						<td width="7%" align="center" class="biaoti">
