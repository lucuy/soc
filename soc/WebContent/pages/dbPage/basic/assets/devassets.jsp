...
function goSearch(num)
  {
	...
	if(keyword != ''|| num !=''){
	       if(flag.value=="快速"){
		      jsonForAjax("${ctx}/devAssets/queryAjaxDevAssets.action?startIndex="+encodeURI(num,"utf-8")+ "&keyword=" + encodeURI(encodeURI(keyword,"utf-8")) + "&t=" + new Date());
		       return ;
		    }
		    if(flag.value=="高级"){
		       extQuery(num);
		...
		jsonForAjax("${ctx}/devAssets/queryAjaxDevAssets.action?t=" + new Date());
	}
  <%--	var msgKey = document.getElementById("goToSearch").value;
  	location.href="${ctx}/devAssets/kquery.action?keyword="+msgKey;
  --%>
  }
...
function jsonForAjax(url){
		 	...
		 	if(item.id != null){
		 		...
		 		if(ShouDingvalue==1){
		 			...
		 			if(ShouDingvalue==1){
		 			    ...
		 			    htmlStr +="<a href=\"${ctx}/devAssets/edit.action?id="+item.id+"\">"+item.devName+"</a>";
		 			    }
		 			htmlStr += "</td>";
		 			htmlStr += "<td  align=\"center\">";
 ...
 function extQueryDlg() {
  ...
  function save()
  {
  	location.href="${ctx}/devAssets/edit.action";
  }
  function deletes() {
    	var ids="";
    	...
    	if(ids=="")
    	{
    		alert("请至少选择一个安全设备，再执行删除！");
    		return;
    	}
    	if(confirm("确认执行操作吗？")) {
    		location.href="${ctx}/devAssets/delete.action?ids=" + ids ;
    	}
    }
...
<s:form  name="devAssets" method="post" action="query" namespace="/devAssets" theme="simple">
<table width="99.5%" border="0" cellspacing="0" cellpadding="0"  >
 ...
 <thead>
   ...
   <s:if test='#session.SSI_LOGIN_Status=="1"'>
    ...
    <th width="53%" align="center" class="biaoti" style="font-size: 12px">设备描述</th>
    <th width="9%" align="center" class="biaoti" onclick="sortAble('table-userManage', 4)" style="cursor:pointer;font-size: 12px">重要程度</th>
...
<!-- ui-dialog -->
	<div id="dialog-extQuery" title="安全设备高级查询">
		<table width="90%" border="0" cellspacing="5" cellpadding="5"
			style="margin-left:20px; font-size:12px;">
			<tr>
				<td width="25%">安全设备名称:</td>
