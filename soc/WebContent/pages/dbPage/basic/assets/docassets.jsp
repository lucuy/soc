...
function goSearch(num)
  {
	...
	if(keyword != ''|| num !=''){
	        if(flag.value=="快速"){
		      jsonForAjax("${ctx}/docAssets/queryAjaxDocAssets.action?startIndex="+encodeURI(num,"utf-8")+ "&keyword=" + encodeURI(encodeURI(keyword,"utf-8")) + "&t=" + new Date());
		       return ;
		    }
		    if(flag.value=="高级"){
		       extQuery(num);
		...
		jsonForAjax("${ctx}/docAssets/queryAjaxDocAssets.action?t=" + new Date());
	}
  }
...
function jsonForAjax(url){
		...
		$.each(result.processLog, function(i,item) {
		 	...
		 	if(item.id != null){
		 		...
		 		if(ShouDingvalue==1){
		 			...
		 			htmlStr += "<td  align=\"center\">";
		 			if(ShouDingvalue==1){
			 			 htmlStr +=item.docName;
			 			}else{
		 			    htmlStr +="<a href=\"${ctx}/docAssets/edit.action?id="+item.id+"\">"+item.docName+"</a>";
		 			    }
		 			htmlStr += "</td>";
		 			htmlStr += "<td  align=\"center\">";
  ...
  function save()
  {
  	location.href="${ctx}/docAssets/edit.action";
  }
  ...
  function deletes() {
    	...
    	if(ids=="")
    	{
    		alert("请至少选择一个安全管理文档，再执行删除！");
    		return;
    	}
    	if(confirm("确认执行操作吗？")) {
    		location.href="${ctx}/docAssets/delete.action?ids=" + ids ;
    	}
    }
  ...
  <input type="hidden" id="flag" value="无"/>
   <s:form  name="docAssets" method="post" action="query" namespace="/docAssets" theme="simple">
 <table width="99.5%" border="0" cellspacing="0" cellpadding="0"  >
  ...
  <s:if test='#session.SSI_LOGIN_Status=="1"'>
    ...
    <th width="15%" align="center" class="biaoti" onclick="sortAble('table-userManage', 1)" style="cursor:pointer;font-size: 12px">文档名称</th>
    <th width="18%" align="center" class="biaoti" onclick="sortAble('table-userManage', 2)" style="cursor:pointer;font-size: 12px" >所属系统</th>
    <th width="51%" align="center" class="biaoti"style="font-size: 12px">文档主要内容</th>
    <th width="11%" align="center" class="biaoti" onclick="sortAble('table-userManage', 4)" style="cursor:pointer;font-size: 12px">重要程度</th>
  </tr>
  </thead>
  <tbody id="tbodyuse">
</tbody>
<tr id="tbodyuseTr"></tr>
  ...
  <!-- ui-dialog -->
	<div id="dialog-extQuery" title="安全管理文档高级查询">
