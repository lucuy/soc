...
jQuery(document).ready(function()
	{
		...
		$("#dialog-extQuery").dialog({
			...
			buttons : {
				...
				"取消[Esc]" : function() {
					$("#hsoftname").val("");$("#hsysname").val("");$("#hcdevname").val("");
					$(this).dialog("close");
				}
			}
		});
	...
	function extQuery(num) {
		...
		var devName = $.trim($("#hcdevname").val());
		
		if(softName=="" && sysName=="" && devName==""){
		  return;
		}
		document.getElementById("goToSearch").value="";//设置快速搜索为空
		document.getElementById("flag").value="高级";//设置标志为高级搜索
		jsonForAjax("${ctx}/compAssets/extQuery.action?startIndex="
				+ encodeURI(num, "utf-8") + "&devName="
				+ encodeURI(encodeURI(devName, "utf-8")) + "&sysName="
				+ encodeURI(encodeURI(sysName, "utf-8")) + "&softName="
				+ encodeURI(encodeURI(softName, "utf-8")) 
				+ "&t=" + new Date());
		//$("#hsoftname").val("");$("#hsysname").val("");$("#hcdevname").val("");
	}
	
	function goSearchBefor(){
	   document.getElementById("hcdevname").value="";
...
function goSearch(num){
	...
	var keyword = $.trim($("#goToSearch").val());
	    if(keyword!=""){
           ...
           var hcdevname=$.trim($("#hcdevname").val());
           var hsysname=$.trim($("#hsysname").val());
           var hsoftname=$.trim($("#hsoftname").val());
         if(hcdevname !="" || hsysname !="" || hsoftname !="" ){
            flag.value="高级";//设置标志为高级搜索
         }
        
         if((keyword=="")&&(hcdevname == "" && hsysname ==""  && hsoftname =="" )){
             flag.value="无";
         }
	...
	if(keyword != ''|| num !=''){
	        if(flag.value=="快速"){
		       jsonForAjax("${ctx}/compAssets/queryAjaxCompAssets.action?startIndex="+encodeURI(num,"utf-8")+ "&keyword=" + encodeURI(encodeURI(keyword,"utf-8")) + "&t=" + new Date());
		       return ;
		    }
		    if(flag.value=="高级"){
		       extQuery(num);
		...
		jsonForAjax("${ctx}/compAssets/queryAjaxCompAssets.action?t=" + new Date());
	}
  }
		 	...
		 	if(item.id != null){
		 		...
		 		if(ShouDingvalue==1){
		 			...
		 			if(ShouDingvalue==1){
		 			    ...
		 			    htmlStr +="<a href=\"${ctx}/compAssets/edit.action?id="+item.id+"\">"+item.devName+"</a>";
		 			    }
		 			htmlStr += "</td>";
		 			htmlStr += "<td  align=\"center\">";
...
function extQueryDlg() {
  ...
  function save()
  {
  	location.href="${ctx}/compAssets/edit.action";
  }
  function deletes() {
    	var ids="";
    	...
    	if(ids=="")
    	{
    		alert("请至少选择一个主机存储设备，再执行删除！");
    		return;
    	}
    	if(confirm("确认执行操作吗？")) {
    		location.href="${ctx}/compAssets/delete.action?ids=" + ids ;
    	}
    }
  ...
  <s:form  name="compAssets" method="post" action="query" namespace="/compAssets" theme="simple">
  ...
  <s:if test='#session.SSI_LOGIN_Status=="1"'>
    ...
    <th width="5%" scope="col" align="center"  class="biaoti"> 
      <input type="checkbox" name="checkbox" id="chkAll" value="checkbox" class="check-box not_checked"/>
    </th>
    </s:else>
    ...
    <th width="18%" align="center" class="biaoti" onclick="sortAble('table-userManage', 2)" style="cursor:pointer;font-size: 12px">所属系统</th>
     <th width="11%" align="center" class="biaoti" onclick="sortAble('table-userManage', 4)" style="cursor:pointer;font-size: 12px">业务应用软件</th>
    <th width="51%" align="center" class="biaoti" style="font-size: 12px">设备描述</th>
...
<!-- ui-dialog -->
	<div id="dialog-extQuery" title="主机存储设备高级查询">
		<table width="90%" border="0" cellspacing="5" cellpadding="5"
			style="margin-left:20px; font-size:12px;">
			<tr>
				<td width="35%">设备名称:</td>
				<td><input id="hcdevname" name="hcdevname" type="text"
